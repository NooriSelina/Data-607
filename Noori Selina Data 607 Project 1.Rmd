---
title: "Noori Selina Data 607 Project 1"
...
head(chess_data)  # Corrected from head(data)
```
...
pair_num <- c()  # Change from player_id to pair_num
player_name <- c()
total_points <- c()
round1 <- c()
round2 <- c()
...
for (x in seq(from = 1, to = nrow(chess_data), by = 3)) {
  ...
  pair_num <- c(pair_num, trimws(row[1]))  # Change from player_id to pair_num
...
chess_table <- data.frame(
  Pair_Num = pair_num,  # Change from Player_ID to Pair_Num
...
player_ratings <- chess_table %>%
  select(Pair_Num, Player_Rating = Total_Points)  # Use "Pair_Num" here

# Merge player_ratings with chess_long based on Pair_Num
chess_long <- chess_long %>%
  left_join(player_ratings, by = "Pair_Num")  # Use "Pair_Num" here

# Calculate the average pre-chess rating of opponents
chess_results <- chess_long %>%
  group_by(Pair_Num, Player_Name, Total_Points, Player_Rating) %>%
